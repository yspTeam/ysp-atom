...
function initEmitter() {
    require('./../common').emitter().on('ysp_toolbar_setProgress', function(show, text, progress) {
        ...
        if (show) {
            document.getElementById("yspRun").removeEventListener("click", runBtnClick);
            document.getElementById("yspRun_span").className = 'loading loading-spinner-tiny inline-block';
            document.getElementById("yspRun_span").style = 'margin-left:-4px;margin-top:-3px;';
        } else {
            document.getElementById("yspRun").addEventListener("click", runBtnClick);
            document.getElementById("yspRun_span").className = 'icon icon-playback-play';
            document.getElementById("yspRun_span").style = 'margin-left:0px;margin-top:0px;';
        }
    });
}
...
function getRunBtn() {
    ...
    <span id='yspRun_span' class='icon icon-playback-play'/>\
    </div>\
    </a>";
}
...
function getInfoHTML() {
    ...
    <span class='inline-block' id='project_info'></span>\
    " + sha + "\
...
function getServerStatu() {
    ...
    return "<span id='server_shining' class='badge icon icon-link' style='height:20px;padding-top:2px'>0</span>";
}
...
function getProgressHTML() {
    return "<div class='block' style='vertical-align: bottom;margin-top:3px'>\
    <progress id='progress' class='inline-block' max='1' value='0' style='display:none;width:100%;margin-right:30px;'></progress>\
    </div>"; //<span id='progress_title' class='inline-block' style='display:none'></span>\
}

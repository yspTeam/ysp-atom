...
class IOSBuild {
	build(inputPath, outputPath) {
		...
		var rootPath = path.dirname(inputPath);
		var weexDirPath = path.join(rootPath,'weex');

		var weexJSPath = path.join(outputPath, 'weex');
		...
		// 编译weex
		this.compileWeex(weexDirPath, weexJSPath);
	}

	compileWeex(weexDirPath, jSPath) {
		if (!this.isPathValid(weexDirPath)) {
			return null;
		}

		if (!fs.existsSync(weexDirPath)) {
			return;
		}

		console.log('weexDirPath,jspath',weexDirPath,jSPath);
		require('child_process').execSync('weex compile ' + weexDirPath + ' ' +jSPath);
	}
	...
	scanRes(resDir, resConfig) {
		...
		if (path.basename(resDir).startsWith('.')) {
			return;
		}

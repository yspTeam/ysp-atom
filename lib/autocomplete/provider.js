...
(function() {
 ...
 linterProvider = require('../file/linter-provider');

  path = require('path');
  unImportHelper = require('../file/unimport.js');
  ...
  module.exports = {
    ...
    loadAppCompletions: function() {
      return fs.readFile(path.resolve(__dirname, '.', 'YYMobile.json'), (function(_this) {
        return function(error, content) {
          var completions, i, len, object, results, suggestion;
          if (error == null) {
    ...
    loadUIKitCompletions: function() {
      return fs.readFile(path.resolve(__dirname, '.', 'UIKitProperty.json'), (function(_this) {
        return function(error, content) {
          var completions, i, len, object, results, suggestion, suggestionSet;
          if (error == null) {
            completions = JSON.parse(content);
          }
          results = [];
          for (i = 0, len = completions.length; i < len; i++) {
            ...
            suggestionSet.type = 'method';
            results.push(_this.all_completions.push(suggestionSet));
          }
          return results;
        };
      })(this));
        ...
        if (arg.type === "ObjectExpression") {
          ...
          for (k = 0, len2 = ref2.length; k < len2; k++) {
            ...
            if (prop.value.type === "FunctionExpression") {
              ...
              // 未导入类处理
              // unImportHelper.parseExpression(prop.value.body);
            }
          }
        }
      }

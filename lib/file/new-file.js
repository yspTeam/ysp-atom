...
export default class NewFile {
  ...
  attach(type) {
    if (!this.panel) {
      ...
      var initialPath = FileUtils.getTreeViewSelectedPath();
      var directoryPath;
      ...
      var relativePath = path.join(relativePath, '/');
  ...
  createNewFile() {
    ...
    if (this.validProjectPath(fullPath)) {
      ...
      if (this.type == 1) {
        jsFileText = this.jsFileText(fileName);
      } else if (this.type == 2) {
        jsFileText = this.protocolFileText(fileName);
      }
  ...
  protocolFileText(name) {
    var text = `defineProtocol('${name}',{})`
    return text;
  }
}
